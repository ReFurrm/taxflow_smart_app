...
export function EstimatedTaxCalculator({ onCalculate }: { onCalculate: (income: number, deductions: number) => void }) {
  ...
  const calculateTax = () => {
    ...
    // State income tax
    const stateInfo = stateTaxData[selectedState];
    const stateTax = stateInfo.estimatedPayments ? netIncome * stateInfo.taxRate : 0;
            ...
            <SelectContent>
              {Object.entries(stateTaxData).map(([code, info]) => (
                <SelectItem key={code} value={code}>
                  {info.name} ({info.taxRate > 0 ? `${(info.taxRate * 100).toFixed(1)}%` : 'No Tax'})
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
            ...
            {calculation.stateTax > 0 && (
              <div className="p-4 bg-green-50 rounded-lg space-y-2">
                <h4 className="font-semibold text-green-900 flex items-center gap-2">
                  ...
                  {stateTaxData[selectedState].name} State Tax
                </h4>
                <div className="flex justify-between text-sm">
